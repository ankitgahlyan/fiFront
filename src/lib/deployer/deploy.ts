...
function buildStateInitData(params: DeployParams): Cell {
	const fiAddr = Address.parse(params.fiJettonAddr.trim());
	const adminAddr = Address.parse(params.admin.trim());
...
function buildMintBody(adminAddr: Address): string {
	...
	const internalTransfer = beginCell()
		...
		.storeCoins(DEFAULTS.forwardTonAmount) // todo:fixme payload, maybeAddrs
		.endCell();
...
export function buildTransaction(params: DeployParams) {
	...
	return {
		validUntil: Math.floor(Date.now()) + 300000, // valid for 5 minutes
		messages: [
			{
