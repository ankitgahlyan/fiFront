...
export class FossFiWallet implements Contract {
	...
	async send(
		...
		message: JettonTransfer | JettonTransferInternal | Slice | null
	) {
...
export function storeFiJettonFullData(src: FiJettonFullData) {
	return (builder: Builder) => {
		...
		if (src.personalMinter !== null && src.personalMinter !== undefined) {
			trustedAddrsBuilder.storeBit(true).storeAddress(src.personalMinter);
		} else {
			trustedAddrsBuilder.storeBit(false);
		}
		if (src.personalJetton !== null && src.personalJetton !== undefined) {
			trustedAddrsBuilder.storeBit(true).storeAddress(src.personalJetton);
		} else {
			trustedAddrsBuilder.storeBit(false);
		}
		trustedAddrsBuilder.storeDict(
			src.authorisedAccs,
...
export function loadFiJettonFullData(slice: Slice) {
	...
	const _authorisedAccs = trustedAddrs.loadDict(
		Dictionary.Keys.Address(),
		Dictionary.Values.Address()
	);
...
export function loadTupleFiJettonFullData(source: TupleReader) {
	...
	const _version = source.readBigNumber();
	const _id = beginCell().storeUint(source.readBigNumber(), 128).endCell(); // todo: cell
...
export function loadGetterTupleAddresses(source: TupleReader) {
	...
	return {
		...
		invitor0: _invitor0,
		minter: _minter,
		personalMinter: _personalMinter,
		personalJetton: _personalJetton,
		authorisedAccs: _authorisedAccs
	};
}
...
export function loadGetterTupleTimestamps(source: TupleReader) {
	...
	return {
		$$type: 'Timestamps' as const,
		accountInit: slice.loadUintBig(0),
		lastClaim: slice.loadUintBig(0),
		lastInvite: slice.loadUintBig(0)
	};
}
...
export function storeTupleFiJettonFullData(source: FiJettonFullData) {
	...
	if (source.personalMinter !== null && source.personalMinter !== undefined) {
		trustedAddrsBuilder.storeBit(true).storeAddress(source.personalMinter);
	} else {
		trustedAddrsBuilder.storeBit(false);
	}
	if (source.personalJetton !== null && source.personalJetton !== undefined) {
		trustedAddrsBuilder.storeBit(true).storeAddress(source.personalJetton);
	} else {
		trustedAddrsBuilder.storeBit(false);
	}
	trustedAddrsBuilder.storeDict(
		source.authorisedAccs,
		Dictionary.Keys.Address(),
		Dictionary.Values.Address()
	);
	const trustedAddrs = trustedAddrsBuilder.endCell();
